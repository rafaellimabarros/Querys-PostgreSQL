...
fatr.created AS data_recebimento,
...
(SELECT bacc.description FROM bank_accounts AS bacc  WHERE fatr.bank_account_id = bacc.id ) 	AS conta_liquidação
...
WHERE fatr.receipt_date BETWEEN '2022-08-01' AND '2022-08-30'
AND fat.title LIKE '%FAT%'
AND fatr.payment_form_id IN (4,5,10,11)

OR fatr.receipt_date BETWEEN '2022-08-01' AND '2022-08-30'

AND fat.origin IN (3,4,7,11) AND fatr.receipt_origin_id IS NULL AND fatr.payment_form_id IN (4,5,10,11)
...
forma_pagamento, operacao_contrato, valor_contrato,fatr.created