...
AND ppg.people_group_id IN (12,13,15,17)
...
(caa.activation_date BETWEEN '2021-08-01' AND DATE(curdate()) AND c.company_place_id != 3
AND ppg.people_group_id IN (12,13,15,17)
AND c.amount > 0 AND c.contract_type_id != 4 AND c.seller_1_id IS NOT NULL
AND ai.incident_type_id IN (1005,1006) AND DATE(a.conclusion_date) >= DATE(c.created) AND ai.incident_status_id = 4)
...
empresa, origem_contato, status_contrato, estagio_contrato
