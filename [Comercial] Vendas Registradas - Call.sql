...
DATE(cpo.modified) AS data_encerramento,
...
WHERE date(cpo.modified) BETWEEN '2023-02-17' AND '2023-02-17'
AND cpo.description IS NOT NULL
AND ( CASE WHEN c.seller_1_id IS NULL THEN (SELECT ppg.people_group_id FROM person_people_groups AS ppg where ppg.person_id = cpo.proprietary_id) IN (12, 13, 15, 19) ELSE  ppg.people_group_id IN (12, 13, 15, 19) end)

OR

date(cpo.modified) BETWEEN '2023-02-17' AND '2023-02-17'
AND cpo.description IS NOT NULL
AND ( CASE WHEN c.seller_1_id IS NULL THEN (SELECT ppg.people_group_id FROM person_people_groups AS ppg where ppg.person_id = cpo.proprietary_id) IN (12, 13, 15, 19) ELSE  ppg.people_group_id IN (12, 13, 15, 19) end)