...
WHERE date(cpo.created) BETWEEN '2022-10-01' AND '2022-10-31'
AND cpo.description IS NOT NULL

/*AND ( CASE WHEN c.seller_1_id IS NULL THEN (SELECT ppg.people_group_id FROM person_people_groups AS ppg where ppg.person_id = cpo.proprietary_id) IN (7, 11, 12, 13, 14, 15, 16, 17, 18, 19) ELSE  ppg.people_group_id IN (7, 11, 12, 13, 14, 15, 16, 17, 18, 19) END)

OR
date(cpo.created) BETWEEN '2022-11-01' AND '2022-11-06'
AND cpo.description IS NOT NULL
AND ( CASE WHEN c.seller_1_id IS NULL THEN (SELECT ppg.people_group_id FROM person_people_groups AS ppg where ppg.person_id = cpo.proprietary_id) IN (7, 11, 12, 13, 14, 15, 16, 17, 18, 19) ELSE  ppg.people_group_id IN (7, 11, 12, 13, 14, 15, 16, 17, 18, 19) END)
*/