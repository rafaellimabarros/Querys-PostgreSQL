SELECT DISTINCT ON (ppli.patrimony_id, ppli.id)
...
INNER JOIN patrimony_occurrences AS po ON po.patrimony_id = pat.id AND po.occurrence_type = 33 AND date(po.created) >= '2023-01-01' 
...
AND ppli.modified_by NOT IN (58,55,35,57)
AND vu.team_id NOT IN (3,1004,1006)

AND ppli.patrimony_id IN
	(
		SELECT
		po.patrimony_id
...
ORDER BY ppli.patrimony_id, ppli.id, po.id ASC;
